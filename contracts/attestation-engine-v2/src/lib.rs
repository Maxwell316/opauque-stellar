...
// `#[contractevent]` macro would change the on-chain event ABI. `register_contract`
// is likewise retained in tests.
#![allow(deprecated)]
...
fn emit_pause_event(env: &Env, action: &str, scope: &str) {
    env.events()
        .publish((Symbol::new(env, action),), Symbol::new(env, scope));
}
...
fn validate_attestation_against_schema(
    ...
    let fields =
        parse_field_definitions(defs_str).map_err(|_| AttestationError::InvalidAttestationData)?;
...
mod property_tests {
    ...
    fn property_revoked_stays_revoked() {
        ...
        assert_ne!(
            att.revocation_ledger, 0u32,
            "revocation_ledger must be non-zero after revoke"
        );
        ...
        assert_eq!(
            att2.revocation_ledger, att.revocation_ledger,
    ...
    fn property_authorization_cannot_be_bypassed() {
        ...
        let result =
            engine_client.try_attest(&stranger, &schema_id, &stealth_hash, &data, &0u32, &ref_uid);
...
mod test {
    ...
    pub(crate) fn setup() -> (
        ...
        let schema_client = schema_registry::SchemaRegistryClient::new(&env, &schema_contract_id);
    ...
    fn test_attest_delegate_authorized() {
        ...
        let ref_uid = BytesN::from_array(&env, &[0u8; 32]);
        let uid =
            engine_client.attest(&delegate, &schema_id, &stealth_hash, &data, &0u32, &ref_uid);
        assert!(uid.to_array() != [0u8; 32]);
    }
    ...
    // NOTE: The former `inactive_schema_rejects_new_attestations` and
    // `existing_valid_attestation_remains_readable` tests targeted a removed
    // `attest(.., registry_id, ..)` signature (the registry is now bound at
    // `initialize`). Their behaviour is covered by the `UnauthorizedRegistry`
    // rejection test and the attest-then-`get_attestation` tests below.
    ...
    fn test_update_config_requires_governance() {
        ...
        engine_client.update_config(
            &authority,
            &Some(new_registry_id.clone()),
            &Some(2u32),
            &None,
        );
        let cfg = engine_client.get_config();
