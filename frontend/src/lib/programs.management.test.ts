...
describe("Soroban management instruction builders", () => {
  ...
  it("maps double-revoke and deprecation errors to user-readable messages", () => {
    expect(mapAttestationRevocationError(new Error("Error(Contract, #5)"))).toBe(
      "This attestation has already been revoked.",
    );

    expect(mapSchemaManagementError(new Error("Error(Contract, #4)"))).toBe(
      "Only the schema authority can deprecate this schema.",
    );
  });
});
